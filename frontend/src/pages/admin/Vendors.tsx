...
import { Check, X, Loader2, ShieldCheck, ShieldX, Video, VideoOff } from "lucide-react";
...
export default function AdminVendors() {
  ...
  const handleVideoPermissionToggle = async (vendorId: number, enabled: boolean) => {
    setActionLoading(vendorId);
    try {
      const res = await fetch(`/api/admin/vendors/${vendorId}/video-permission`, {
        method: "PATCH",
      ...
      if (data.ok) {
        setVendors((prev) =>
          prev.map((v) => (v.id === vendorId ? { ...v, videoConsultationEnabled: enabled } : v))
        );
        toast.success(`Video consultation ${enabled ? "enabled" : "disabled"}`);
      } else {
        toast.error(data.error || "Failed to update video permission");
      }
    } catch {
      toast.error("Failed to update video permission");
    } finally {
      setActionLoading(null);
    }
  };
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        ...
                        <th className="text-left px-5 py-3 font-medium text-muted-foreground text-xs">Video</th>
                        <th className="text-left px-5 py-3 font-medium text-muted-foreground text-xs">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {approved.map((v, i) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3">
                            <button
                              onClick={() => handleVideoPermissionToggle(v.id, !v.videoConsultationEnabled)}
                              disabled={actionLoading === v.id}
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs transition-colors disabled:opacity-50 ${
                                v.videoConsultationEnabled
                                  ? "bg-purple-500/10 text-purple-400 hover:bg-purple-500/20 border-purple-500/20"
                                  : "bg-secondary text-muted-foreground hover:bg-muted border-border"
                              }`}
                              title="Toggle video consultation"
                            >
                              {v.videoConsultationEnabled ? <Video size={12} /> : <VideoOff size={12} />}
                              {v.videoConsultationEnabled ? "On" : "Off"}
                            </button>
                          </td>
                          <td className="px-5 py-3">
                            <button
