...
import {
  ...
  haversineDistanceKm,
  logEmergencyEvent,
  updateEmergencyRequestLocation,
} from "./db/emergency";

const PORT = Number(process.env.PORT ?? 4000);

const AUTH_COOKIE_NAME = "auth_token";
const ZEGO_APP_ID = 2098662784;
const ZEGO_SERVER_SECRET = "ec87217166d6eb65aabfaf56bc4774bd";
...
function readTokenFromCookieHeader(cookieHeader: string | undefined): string | null {
...
function getAuthUserFromSocket(socket: Socket): SessionUser | null {
  const authToken =
...
function generateZegoKitToken(
  ...
  // Match Zego's SDK implementation: AES(payload) + binary packing prefixed with "04"
  ...
  const ivBytes = Uint8Array.from(Buffer.from(iv, "utf8"));
  const out = new Uint8Array(28 + encryptedBytes.length);
  out.set([0, 0, 0, 0], 0);

  const expireBytesLE = new Uint8Array(Int32Array.from([payload.expire]).buffer);
  out.set(Uint8Array.from(Array.from(expireBytesLE).reverse()), 4);

  out[8] = iv.length >> 8;
  out[9] = iv.length - (out[8] << 8);
  out.set(ivBytes, 10);

  out[26] = encryptedBytes.length >> 8;
  out[27] = encryptedBytes.length - (out[26] << 8);
  out.set(encryptedBytes, 28);

  return `04${Buffer.from(out).toString("base64")}`;
}
...
async function start() {
  ...
  io.use((socket, next) => {
    const user = getAuthUserFromSocket(socket);
    if (!user) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.emit("emergency:connected", { ok: true, userId: user.id, role: user.role });
  });

  type EmergencyDispatchState = {
    currentRadiusKm: number;
    retryCount: number;
    timer: NodeJS.Timeout | null;
    notifiedVendorUserIds: Set<number>;
  };

  const emergencyDispatches = new Map<number, EmergencyDispatchState>();

  function clearEmergencyDispatch(requestId: number): void {
    const state = emergencyDispatches.get(requestId);
    if (state?.timer) clearTimeout(state.timer);
    emergencyDispatches.delete(requestId);
  }
  ...
  try {
    ...
    // @ts-ignore - JS router lives outside backend TS project include and is loaded at runtime.
  ...
  // Save vendor live location (used for emergency distance matching)
  app.post("/api/vendor/location", async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user || user.role !== "vendor") {
      res.status(403).json({ error: "Forbidden: vendors only" });
      return;
    ...
    const latitude = asCoordinate(req.body?.latitude);
    const longitude = asCoordinate(req.body?.longitude);
    ...
    try {
      ...
      res.status(200).json({ ok: true, vendorId, latitude, longitude });
    } catch (err: any) {
      res.status(500).json({ error: err?.message ?? "Failed to save vendor location" });
    }
  });
  ...
  // Generate video token for booking participants (customer/vendor only)
  app.get("/api/bookings/:bookingId/video-token", async (req, res) => {
    const user = getAuthUserFromRequest(req);
    ...
    try {
      ...
      const isCustomerOwner = user.role === "customer" && booking.customer_id === user.id;
      const isVendorOwner = user.role === "vendor" && booking.vendor_user_id === user.id;
      ...
      const allowedPaymentStatuses = new Set(["paid", "completed", "success"]);
      if (!allowedPaymentStatuses.has(paymentStatus)) {
        res.status(400).json({ error: "Payment is not completed for this booking" });
        return;
      }

      if (!booking.video_consultation_enabled) {
        res.status(403).json({ error: "Video consultation is not enabled for this vendor" });
        return;
      }
      ...
      // Notify the other party when someone joins/waits in the video call.
      try {
        if (user.role === "vendor") {
          await createNotification(
            booking.customer_id,
            `${userName} is waiting for you in the video call for "${booking.service_title}". Go to My Bookings and click Join Video Call for Booking #${bookingId}.`
          );
        } else if (user.role === "customer") {
          await createNotification(
            booking.vendor_user_id,
            `${userName} is waiting for you in the video call for "${booking.service_title}". Go to Bookings and click Start Video Call for Booking #${bookingId}.`
          );
        }
      } catch (_) {
        // Non-critical: token generation should still succeed.
      }

      res.status(200).json({ ok: true, appId: ZEGO_APP_ID, token, roomId, userId, userName });
    } catch (err: any) {
      res.status(400).json({ error: err?.message ?? "Failed to generate video token" });
    }
  });
  ...
  app.get("/api/admin/vendors", async (req, res) => {
    ...
    try {
      const pool = getPool();
      const { rows } = await pool.query(
        `SELECT v.id, v.business_name, v.is_verified, v.video_consultation_enabled, v.created_at,
      ...
      res.status(200).json({
        ...
        vendors: rows.map((r: any) => ({
          ...
          videoConsultationEnabled: r.video_consultation_enabled,
          createdAt: r.created_at,
  ...
  // Toggle vendor video consultation permission (admin only)
  app.patch("/api/admin/vendors/:id/video-permission", async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user || user.role !== "admin") {
      res.status(403).json({ error: "Forbidden: admins only" });
      return;
    ...
    try {
      const pool = getPool();
      const result = await pool.query<{ user_id: number; business_name: string | null }>(
        `UPDATE vendors
         SET video_consultation_enabled = $1
         WHERE id = $2
      ...
      // Notify vendor about video consultation permission change.
      try {
        const vendorUserId = result.rows[0].user_id;
        const vendorLabel = result.rows[0].business_name ? ` for "${result.rows[0].business_name}"` : "";
        const message = enabled
          ? `Video consultation has been enabled${vendorLabel}. You can now accept video consultations.`
          : `Video consultation has been disabled${vendorLabel}.`;
        await createNotification(vendorUserId, message);
      } catch (_) {
        // Non-critical: permission update should still succeed even if notification fails.
      }

      res.status(200).json({ ok: true });
    } catch (err: any) {
      res.status(400).json({ error: err?.message ?? "Failed to update video permission" });
    }
  });
...
${context}`;
  ...
  const emergencyRequestPaths = ["/api/emergency-request", "/emergency-request"];
  const emergencyAcceptPaths = ["/api/accept-emergency", "/accept-emergency"];
  const emergencyStatusPaths = ["/api/emergency-status/:id", "/emergency-status/:id"];
  const emergencyLocationPaths = ["/api/emergency-location", "/emergency-location"];
  const emergencyCompletePaths = ["/api/emergency-complete", "/emergency-complete"];
  const INITIAL_RADIUS_KM = 5;
  const RETRY_RADIUS_KM = 10;
  const RETRY_WAIT_MS = 30000;

  const notifyVendorsForEmergency = async (params: {
    requestId: number;
    userId: number;
    issue: string;
    latitude: number;
    ...
    const dispatch = emergencyDispatches.get(params.requestId) ?? {
      currentRadiusKm: params.radiusKm,
      retryCount: 0,
      timer: null,
      notifiedVendorUserIds: new Set<number>(),
    };
    emergencyDispatches.set(params.requestId, dispatch);

    dispatch.currentRadiusKm = params.radiusKm;

    const candidates = await findNearbyVendors({
      latitude: params.latitude,
      longitude: params.longitude,
      radiusKm: params.radiusKm,
    });

    const targets = candidates.filter((vendor) => !dispatch.notifiedVendorUserIds.has(vendor.userId));

    for (const vendor of targets) {
      dispatch.notifiedVendorUserIds.add(vendor.userId);

      await createNotification(
        vendor.userId,
        `Emergency request #${params.requestId}: ${params.issue} (${vendor.distanceKm.toFixed(2)} km away).`
      );

      io.to(`vendor-user:${vendor.userId}`).emit("emergency:new-request", {
        ...
        distanceKm: Number(vendor.distanceKm.toFixed(2)),
        vendorLatitude: vendor.latitude,
        vendorLongitude: vendor.longitude,
        vendorId: vendor.vendorId,
        vendorBusinessName: vendor.businessName,
        vendorRating: Number(vendor.rating.toFixed(2)),
        vendorAvailable: vendor.isAvailable,
        radiusKm: params.radiusKm,
      });
      ...
      await logEmergencyEvent({
        ...
        vendorId: vendor.vendorId,
        message: `Vendor notified in ${params.phase} search`,
        metadata: {
          phase: params.phase,
          radiusKm: params.radiusKm,
          distanceKm: Number(vendor.distanceKm.toFixed(3)),
    ...
    return {
      ...
      notifiedNow: targets,
      totalNotified: dispatch.notifiedVendorUserIds.size,
    };
  };

  const scheduleEmergencyRetry = (params: {
    requestId: number;
    userId: number;
    issue: string;
    latitude: number;
    ...
    dispatch.timer = setTimeout(async () => {
      try {
        const current = await getEmergencyRequestStatus(params.requestId);
        if (!current || current.status !== "pending") {
          clearEmergencyDispatch(params.requestId);
          return;
        }

        dispatch.retryCount += 1;
        io.to(`user:${params.userId}`).emit("emergency:reassigning", {
          requestId: params.requestId,
          message: "No acceptance yet. Expanding search radius to 10 km.",
          nextRadiusKm: RETRY_RADIUS_KM,
        });

        await logEmergencyEvent({
          emergencyRequestId: params.requestId,
          eventType: "search_expanded",
          message: "Expanded vendor search radius",
          metadata: {
            fromRadiusKm: dispatch.currentRadiusKm,
            toRadiusKm: RETRY_RADIUS_KM,
            retryCount: dispatch.retryCount,
          },
        });

        const retryResult = await notifyVendorsForEmergency({
          ...params,
          radiusKm: RETRY_RADIUS_KM,
          phase: "retry",
        });

        if (retryResult.notifiedNow.length === 0 && retryResult.totalNotified === 0) {
          io.to(`user:${params.userId}`).emit("emergency:no-vendor", {
            requestId: params.requestId,
            message: "No vendors available right now, please stay safe and try again shortly.",
          });
          await logEmergencyEvent({
            emergencyRequestId: params.requestId,
            eventType: "no_vendor_found",
            message: "No vendors available even after radius expansion",
            metadata: {
              radiusKm: RETRY_RADIUS_KM,
            },
          });
        ...
        // eslint-disable-next-line no-console
        console.error("[emergency retry] failed", error);
      }
    }, RETRY_WAIT_MS);
  };

  app.post(emergencyRequestPaths, async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user || user.role !== "customer") {
      res.status(403).json({ error: "Forbidden: customers only" });
      return;
    ...
    if (latitude == null || longitude == null || !issue) {
      res.status(400).json({ error: "latitude, longitude and issue are required" });
      return;
    }
    ...
    try {
      ...
      await createNotification(
        user.id,
        `Emergency request #${emergency.id} created. We are notifying nearby vendors.`
      );

      emergencyDispatches.set(emergency.id, {
        currentRadiusKm: INITIAL_RADIUS_KM,
        retryCount: 0,
        timer: null,
        notifiedVendorUserIds: new Set<number>(),
      });

      const initialNotify = await notifyVendorsForEmergency({
      ...
      if (initialNotify.notifiedNow.length === 0) {
        io.to(`user:${user.id}`).emit("emergency:no-vendor", {
          requestId: emergency.id,
          message: "No nearby vendors within 5 km. We will retry with a wider radius shortly.",
        });

        await createNotification(
          user.id,
          `Emergency request #${emergency.id}: no nearby vendor within 5 km yet. Retrying with wider radius.`
        );
      }
      ...
      res.status(201).json({
        ...
        retryInSeconds: RETRY_WAIT_MS / 1000,
        searchRadiusKm: INITIAL_RADIUS_KM,
        nearbyVendors: initialNotify.allCandidates.map((vendor) => ({
          vendorId: vendor.vendorId,
      ...
      res.status(500).json({ error: err?.message ?? "Failed to create emergency request" });
    }
  });

  app.get("/api/vendor/emergency-feed", async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user || user.role !== "vendor") {
      res.status(403).json({ error: "Forbidden: vendors only" });
      return;
    ...
    try {
      ...
      const vendorResult = await pool.query<{ latitude: number | null; longitude: number | null }>(
        `SELECT latitude, longitude FROM vendors WHERE id = $1 LIMIT 1`,
        [vendorId]
      );

      const vendor = vendorResult.rows[0];
      if (!vendor || vendor.latitude == null || vendor.longitude == null) {
        res.status(200).json({ ok: true, requests: [], reason: "vendor_location_missing" });
        return;
      }
      ...
      const requestsResult = await pool.query<{
        ...
        `SELECT id, user_id, issue, latitude, longitude, created_at
      ...
      const requests = requestsResult.rows
        .map((row) => ({
          ...
          distanceKm: haversineDistanceKm(
            vendor.latitude as number,
            vendor.longitude as number,
            row.latitude,
            row.longitude
          ),
          vendorLatitude: vendor.latitude,
        ...
        .filter((row) => row.distanceKm <= RETRY_RADIUS_KM)
      ...
      res.status(200).json({ ok: true, requests, radiusKm: RETRY_RADIUS_KM });
    } catch (err: any) {
      res.status(500).json({ error: err?.message ?? "Failed to load vendor emergency feed" });
    }
  });

  app.post(emergencyAcceptPaths, async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user || user.role !== "vendor") {
      res.status(403).json({ error: "Forbidden: vendors only" });
      return;
    ...
    try {
      const vendorId = await getVendorIdByUserId(user.id);
      const accepted = await acceptEmergencyRequest({ requestId, vendorId });

      if (!accepted) {
        res.status(409).json({ error: "This emergency request is no longer available" });
        return;
      }

      clearEmergencyDispatch(requestId);

      await createNotification(
        accepted.userId,
        `Emergency request #${accepted.id} accepted by ${accepted.vendorBusinessName || user.name || "a vendor"}${accepted.vendorPhoneNumber ? ` (Phone: ${accepted.vendorPhoneNumber})` : ""}.`
      );
      await createNotification(
        user.id,
        `You accepted emergency request #${accepted.id}.`
      );

      io.to(`user:${accepted.userId}`).emit("emergency:assigned", {
        requestId: accepted.id,
        status: accepted.status,
        vendorId: accepted.vendorId,
        vendorUserId: user.id,
        vendorBusinessName: accepted.vendorBusinessName || user.name || "Assigned vendor",
        vendorPhoneNumber: accepted.vendorPhoneNumber,
        vendorLatitude: accepted.vendorLatitude,
        vendorLongitude: accepted.vendorLongitude,
        userLatitude: accepted.latitude,
        userLongitude: accepted.longitude,
        issue: accepted.issue,
      });

      io.emit("emergency:closed", {
        requestId: accepted.id,
        assignedVendorId: accepted.vendorId,
      });

      res.status(200).json({ ok: true, request: accepted });
    } catch (err: any) {
      res.status(500).json({ error: err?.message ?? "Failed to accept emergency request" });
    }
  });

  app.get(emergencyStatusPaths, async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user) {
      res.status(401).json({ error: "Unauthorized" });
      return;
    ...
    if (!Number.isInteger(requestId) || requestId <= 0) {
      res.status(400).json({ error: "Invalid emergency request id" });
      return;
    }

    try {
      const status = await getEmergencyRequestStatus(requestId);
      if (!status) {
      ...
      if (status.status !== "pending") {
        clearEmergencyDispatch(status.id);
      }

      const isOwner = status.userId === user.id;
      const isAssignedVendor = user.role === "vendor" && status.vendorId != null
        ? (await getVendorIdByUserId(user.id)) === status.vendorId
        : false;

      if (!isOwner && !isAssignedVendor && user.role !== "admin") {
        res.status(403).json({ error: "Forbidden" });
        return;
      }

      res.status(200).json({ ok: true, request: status });
    } catch (err: any) {
      res.status(500).json({ error: err?.message ?? "Failed to fetch emergency status" });
    }
  });

  app.post(emergencyLocationPaths, async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user || user.role !== "customer") {
      res.status(403).json({ error: "Forbidden: customers only" });
      return;
    ...
    if (!Number.isInteger(requestId) || requestId <= 0 || latitude == null || longitude == null) {
      res.status(400).json({ error: "requestId, latitude and longitude are required" });
      return;
    }

    try {
      const updated = await updateEmergencyRequestLocation({
        requestId,
        userId: user.id,
        latitude,
        longitude,
      ...
      if (updated.vendorId) {
        const pool = getPool();
        const vendorUser = await pool.query<{ user_id: number }>(
          "SELECT user_id FROM vendors WHERE id = $1 LIMIT 1",
          [updated.vendorId]
        );
        const vendorUserId = vendorUser.rows[0]?.user_id;
        if (vendorUserId) {
          io.to(`vendor-user:${vendorUserId}`).emit("emergency:location-updated", {
  ...
  app.post(emergencyCompletePaths, async (req, res) => {
    const user = getAuthUserFromRequest(req);
    if (!user) {
      res.status(401).json({ error: "Unauthorized" });
      return;
    ...
    try {
      const vendorId = user.role === "vendor" ? await getVendorIdByUserId(user.id) : undefined;
      const completed = await completeEmergencyRequest({
        requestId,
      ...
      if (!completed) {
        res.status(409).json({ error: "Unable to complete emergency request" });
        return;
      }
      ...
      await createNotification(
        completed.userId,
        `Emergency request #${completed.id} marked as completed.`
      );

      io.to(`user:${completed.userId}`).emit("emergency:completed", {
        requestId: completed.id,
        status: completed.status,
      });

      if (completed.vendorId) {
        const pool = getPool();
        const vendorOwner = await pool.query<{ user_id: number }>(
          "SELECT user_id FROM vendors WHERE id = $1 LIMIT 1",
          [completed.vendorId]
        );
        const vendorUserId = vendorOwner.rows[0]?.user_id;
        if (vendorUserId) {
          await createNotification(
            vendorUserId,
            `Emergency request #${completed.id} marked as completed.`
          );
          io.to(`vendor-user:${vendorUserId}`).emit("emergency:completed", {
            requestId: completed.id,
            status: completed.status,
          });
        }
      ...
      res.status(200).json({ ok: true, request: completed });
    } catch (err: any) {
      res.status(500).json({ error: err?.message ?? "Failed to complete emergency request" });
    }
  });
