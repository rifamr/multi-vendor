...
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS shop_image_url TEXT;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS video_consultation_enabled BOOLEAN DEFAULT FALSE;

-- Location hierarchy columns
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS state VARCHAR;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS district VARCHAR;
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS city VARCHAR;
...
CREATE TABLE IF NOT EXISTS emergency_requests (
  ...
  CONSTRAINT emergency_requests_user_fk FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  CONSTRAINT emergency_requests_vendor_fk FOREIGN KEY (vendor_id) REFERENCES vendors(id) ON DELETE SET NULL,
  CONSTRAINT emergency_requests_status_check CHECK (status IN ('pending', 'accepted', 'completed'))
);

CREATE TABLE IF NOT EXISTS emergency_request_events (
  id               INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  ...
  metadata         JSONB,
  created_at       TIMESTAMP DEFAULT NOW(),
  CONSTRAINT emergency_request_events_request_fk
    FOREIGN KEY (emergency_request_id) REFERENCES emergency_requests(id) ON DELETE CASCADE,
  CONSTRAINT emergency_request_events_vendor_fk
...
CREATE INDEX IF NOT EXISTS idx_emergency_user_id ON emergency_requests(user_id);
CREATE INDEX IF NOT EXISTS idx_emergency_vendor_id ON emergency_requests(vendor_id);
CREATE INDEX IF NOT EXISTS idx_emergency_status_created_at ON emergency_requests(status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_emergency_events_request ON emergency_request_events(emergency_request_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_emergency_events_type ON emergency_request_events(event_type);
